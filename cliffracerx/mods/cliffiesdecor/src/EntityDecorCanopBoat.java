...
public class EntityDecorCanopBoat extends Entity
{
    ...
    public String[] colors = {"rBoat", "oBoat", "yBoat", "lBoat", "gBoat", "cBoat", "bBoat", "pBoat"};
    private int sleepingTimer;
    private EntityPlayer ridingPlayer;

    public EntityDecorCanopBoat(World par1World)
    {
        super(par1World);
    ...
    protected void entityInit()
    {
        ...
        this.dataWatcher.addObject(20, this.color);
    ...
    public EntityDecorCanopBoat(World par1World, double par2, double par4, double par6, int id, int color)
    {
        this(par1World);
    ...
    {
        return (double)this.height * 0.0D - 0.50000001192092896D;
    }
    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, float par2)
    {
        ...
        else if (!this.worldObj.isRemote && !this.isDead)
        {
            ...
            if (flag || this.getDamageTaken() > 40.0F)
            {
                ...
                if (!flag)
                {
                    this.dropItemWithOffset(this.itemID, 1, 0.0F);
                }
    ...
    ArrayList positions = new ArrayList<Pos3>();
    ...
    public void onUpdate()
    {
        ...
        {
            //EnumStatus enumstatus = this.ridingPlayer.sleepInBedAt((int) this.posX, (int) this.posY-1, (int) this.posZ);
            this.setSleepTime(this.getSleepTime()-1);
            this.sleepingTimer=0;
            System.out.println(this.ridingPlayer.getSleepTimer());
            this.ridingPlayer.setPosition(this.posX-1, this.posY+0.5, this.posZ);
            this.ridingPlayer.rotationYaw=this.rotationYaw;
            this.ridingPlayer.rotationPitch=this.rotationPitch;
            Pos3 var1 = new Pos3(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY) + 1, MathHelper.floor_double(this.posZ));
            this.setSize(0.1F, 0.8F);
            if(this.ridingPlayer.sleepTimer>=100)
            {
                this.setSleepTime(0);
                this.sleepingTimer=0;
                this.setSize(2.0F, 0.8F);
        ...
        else
        {
            ...
            if (this.worldObj.getBlockId(var1.x, var1.y, var1.z) == 0)
            {
                this.worldObj.setBlock(var1.x, var1.y, var1.z, DecorMod.lightSource.blockID);
            }
            
            positions.add(var1);
            //DecorMod.entityIdLastPosMap.put(Integer.valueOf(this.entityId), var1);
        }
        
        for(int i=0; i<positions.size(); i++)
        {
            //if (DecorMod.entityIdLastPosMap.containsKey(Integer.valueOf(this.entityId)))
            {
                Pos3 var2 = (Pos3) positions.get(i);
                Pos3 var1 = new Pos3(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY) + 1, MathHelper.floor_double(this.posZ));

                if ((var2.x != var1.x || var2.y != var1.y || var2.z != var1.z))
                {
                    this.worldObj.setBlock(var2.x, var2.y, var2.z, 0);
                }
            }
        }
            ...
            if (!this.worldObj.isRemote)
            {
                ...
                if (list != null && !list.isEmpty())
                {
                    for (l = 0; l < list.size(); ++l)
                    {
                        ...
                        if (entity != this.riddenByEntity && entity.canBePushed() && entity instanceof EntityDecorCanopBoat)
                        {
    ...
    protected void writeEntityToNBT(NBTTagCompound par1NBTTagCompound) 
    {
        ...
        par1NBTTagCompound.setInteger("itemid", this.itemID);
    }
    ...
    public boolean interactFirst(EntityPlayer par1EntityPlayer)
    {
        ...
        else
        {
            ...
            else
            {
                //if (!this.worldObj.isRemote)
                {
                    EnumStatus enumstatus = par1EntityPlayer.sleepInBedAt((int) this.posX, (int) this.posY+1, (int) this.posZ);
                    this.sleepingTimer=50000;
                    this.setSleepTime(50000);
                    this.ridingPlayer=par1EntityPlayer;
                }
    ...
    public void setSleepTime(int par1)
    {
