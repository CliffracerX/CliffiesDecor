...
{
    public int color;
    public ItemNewBoat(int par1, int par2, String par3)
    {
        super(par1);
        this.maxStackSize = 1;
        this.color=par2;
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
                {
                    ...
                    EntityCustomDecorBoat EntityCustomDecorBoat = new EntityCustomDecorBoat(par2World, (double)((float)i + 0.5F), (double)((float)j + 1.0F), (double)((float)k + 0.5F), this.itemID, this.color);
                    EntityCustomDecorBoat.rotationYaw = (float)(((MathHelper.floor_double((double)(par3EntityPlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!par2World.getCollidingBoundingBoxes(EntityCustomDecorBoat, EntityCustomDecorBoat.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return par1ItemStack;
                    }
                    ...
                    {
                        par2World.spawnEntityInWorld(EntityCustomDecorBoat);
                    }
